...
function App() {
  return <NoticeBar content="这是一条通知消息" />;
}
```
...
function App() {
  return <NoticeBar content="这是一条通知消息" icon={<InfoCircleOutline />} />;
}
```
...
function App() {
  ...
  return (
    <>
      {visible && (
        <NoticeBar content="这是一条通知消息" closeable onClose={() => setVisible(false)} />
      )}
    </>
  );
}
...
function App() {
  return <NoticeBar content="这是一条很长的通知消息，会自动滚动显示" color="alert" wrap />;
}
```
