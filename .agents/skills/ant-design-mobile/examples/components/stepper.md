...
function App() {
  ...
  return <Stepper value={value} onChange={(val) => setValue(val)} min={0} max={10} />;
}
```
...
function App() {
  ...
  return <Stepper value={value} onChange={(val) => setValue(val)} step={2} />;
}
```
...
function App() {
  ...
  return (
    <>
      <Stepper value={value} onChange={(val) => setValue(val)} disabled />
      <Stepper value={value} onChange={(val) => setValue(val)} min={0} max={10} />
    </>
  );
}
```
...
function App() {
  ...
  return (
    <Stepper value={value} onChange={(val) => setValue(val)} formatter={(value) => `${value}件`} />
  );
}
```
