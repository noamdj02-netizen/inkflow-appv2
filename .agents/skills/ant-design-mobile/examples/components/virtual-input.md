...
function App() {
  ...
  return <VirtualInput value={value} onChange={setValue} placeholder="请输入" />;
}
```
...
function App() {
  ...
  return (
    <>
      <VirtualInput value={value} onChange={setValue} type="number" placeholder="数字键盘" />
      <VirtualInput value={value} onChange={setValue} type="password" placeholder="密码键盘" />
    </>
  );
}
```
