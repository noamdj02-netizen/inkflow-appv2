...
function App() {
  ...
  return (
    <List>
      <List.Item extra={<Switch checked={checked} onChange={(checked) => setChecked(checked)} />}>
        开关
      </List.Item>
    </List>
  );
...
function App() {
  ...
  return (
    <>
      ...
      <Switch checked={checked} onChange={(checked) => setChecked(checked)} color="primary" />
      <Switch checked={checked} onChange={(checked) => setChecked(checked)} color="#ff6b6b" />
    </>
  );
}
```
