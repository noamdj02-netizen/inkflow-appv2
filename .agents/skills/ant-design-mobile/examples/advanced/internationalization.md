...
function App() {
  return <ConfigProvider locale={zhCN}>{/* Your app content */}</ConfigProvider>;
}
```
...
function App() {
  ...
  return (
    <ConfigProvider locale={locale}>
      <Button onClick={() => setLocale(locale === zhCN ? enUS : zhCN)}>切换语言</Button>
      {/* Your app content */}
    </ConfigProvider>
  );
}
...
function App() {
  return <ConfigProvider locale={customLocale}>{/* Your app content */}</ConfigProvider>;
}
```
