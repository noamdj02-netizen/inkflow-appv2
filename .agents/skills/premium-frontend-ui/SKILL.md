...
## InkFlow — règles Pro UI (rigueur)

For **InkFlow** in this repo, combine immersive craft with **strict product UI tokens** (not talent, discipline):

- **One shadow** everywhere: `shadow-pro` / CSS `var(--pro-shadow)`.
- **Radii:** `rounded-pro-btn` (8px), `rounded-pro-card` (12px), `rounded-pro-modal` (16px).
- **Spacing** on multiples of 4px: Tailwind `pro-1` … `pro-7` or `gap-4` / `p-4` (never 13, 21, 27).
- **Three text levels** per view: `pro-text-title`, `pro-text-body`, `pro-text-small`.
- **Two font weights** on pro surfaces: `font-pro-regular` (400) + `font-pro-medium` (500).
- **Brand accent** `#2563eb` (blue-600) + **white** on primary CTAs (`--primary`, `bg-pro-cta`, `Button` default) — InkFlow studio app; never random rainbow for primary actions.
- **Alignment:** `pro-container` for shared horizontal padding / max width.

Details: `index.css` (PRO UI block) and `tailwind.config.ts`.
