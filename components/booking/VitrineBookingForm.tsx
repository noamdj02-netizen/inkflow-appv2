...
const inputDark = 'border border-zinc-700 bg-zinc-800/50 text-white placeholder:text-zinc-400 focus:ring-violet-500 focus:border-zinc-600';
...
function ProjectRecapCard({
  ...
  const placementLabel = BODY_PLACEMENT_OPTIONS.find((o) => o.value === placement)?.label || placement;
  return (
    <div
    ...
    >
      <p className={`text-xs font-semibold uppercase tracking-wider mb-3 ${isDark ? 'text-zinc-500' : 'text-neutral-400'}`}>
        Récap de ton projet
      </p>
      <div className="flex flex-col gap-1.5">
        {name && (
          <div className="flex items-center gap-2">
            <User className={`w-3.5 h-3.5 flex-shrink-0 ${isDark ? 'text-zinc-500' : 'text-neutral-400'}`} />
            <span className={`text-sm font-medium ${isDark ? 'text-zinc-200' : 'text-neutral-700'}`}>{name}</span>
          </div>
        )}
        {description && (
          <div className="flex items-start gap-2">
            <FileText className={`w-3.5 h-3.5 flex-shrink-0 mt-0.5 ${isDark ? 'text-zinc-500' : 'text-neutral-400'}`} />
            <span className={`text-sm line-clamp-2 ${isDark ? 'text-zinc-300' : 'text-neutral-600'}`}>{description}</span>
          </div>
        )}
        <div className="flex flex-wrap gap-2 mt-1">
          {placementLabel && (
            <span className={`inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full font-medium ${isDark ? 'bg-zinc-700 text-zinc-300' : 'bg-neutral-200 text-neutral-600'}`}>
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input {...register('clientName')} type="text" placeholder="Jean Dupont" className={inputCls} />
            ...
            <div>
              ...
              <input {...register('clientEmail')} type="email" placeholder="jean@exemple.com" className={inputCls} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input {...register('estimatedSizeCm')} type="text" placeholder="Ex : 10" className={inputCls} />
            </div>
          </div>
          ...
          <div>
            ...
            <div
              ...
              } ${isDragging ? (isDark ? 'bg-zinc-800 border-zinc-500' : 'bg-neutral-100 border-neutral-400') : ''}`}
            >
              <input id="ref-images-input" type="file" accept="image/*" multiple onChange={handleFileSelect} className="hidden" />
              <UploadCloud className={`w-9 h-9 mx-auto mb-2.5 ${isDark ? 'text-zinc-500' : 'text-neutral-400'}`} strokeWidth={1.5} />
              <p className={`text-sm ${isDark ? 'text-zinc-400' : 'text-neutral-500'}`}>
                Glissez vos images ici ou{' '}
                <span className={`font-semibold underline ${isDark ? 'text-zinc-300' : 'text-neutral-700'}`}>cliquez pour parcourir</span>
              </p>
              <p className={`text-xs mt-1 ${isDark ? 'text-zinc-600' : 'text-neutral-400'}`}>Optionnel · 10 images max</p>
            </div>
            {referenceImages.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {referenceImages.map((file, i) => (
                  <div
                    key={i}
                    className={`relative flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm ${isDark ? 'bg-zinc-800 text-zinc-300' : 'bg-neutral-100 text-neutral-700'}`}
                  >
            ...
            {onCancel && (
              <button
                ...
                className={`px-6 py-3 border-2 rounded-xl font-semibold transition-colors ${
                  isDark ? 'border-zinc-700 text-zinc-300 hover:border-zinc-500' : 'border-neutral-200 hover:border-neutral-900'
                }`}
              >
                Annuler
              </button>
      ...
      {step === 2 && (
        <div className="space-y-5">
            ...
            {availabilityLoading ? (
              ...
              <div className={`rounded-xl border ${isDark ? 'border-zinc-700 bg-zinc-800/30' : 'border-neutral-200 bg-neutral-50'} p-4`}>
                ...
                <div className="grid grid-cols-7 gap-1 mb-1">
                  {WEEKDAYS.map((d) => (
                    <div key={d} className={`text-center text-xs font-semibold py-1 ${isDark ? 'text-zinc-600' : 'text-neutral-400'}`}>
                      {d}
                    </div>
                  ))}
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`relative aspect-square rounded-lg text-sm font-medium transition-all ${
                          disabled
                            ? isDark ? 'text-zinc-700 cursor-not-allowed' : 'text-neutral-300 cursor-not-allowed'
                            : selected
                            ? isDark ? 'bg-violet-600 text-white shadow-lg shadow-violet-500/20' : 'bg-neutral-900 text-white shadow-md'
                            : isDark ? 'text-zinc-300 hover:bg-zinc-700' : 'text-neutral-700 hover:bg-neutral-200'
                        }`}
                      >
                        {d.getDate()}
                        {isToday && !selected && (
                          <span
                            className={`absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full ${
                              disabled ? (isDark ? 'bg-zinc-700' : 'bg-neutral-300') : isDark ? 'bg-violet-500' : 'bg-neutral-600'
                            }`}
                          />
                        )}
                      </button>
          ...
          <div>
            ...
            {!requestedDate ? (
              <p className={`text-sm italic ${isDark ? 'text-zinc-500' : 'text-neutral-400'}`}>Sélectionnez d&apos;abord une date.</p>
            ) : availableSlotsForSelected.length === 0 ? (
              <p className={`text-sm ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>Aucun créneau disponible à cette date.</p>
            ) : (
                ...
                {availableSlotsForSelected.map((time) => (
                  <button
                    ...
                    className={`px-4 py-2.5 rounded-xl font-semibold text-sm transition-all ${
                      requestedTime === time
                        ? isDark ? 'bg-violet-600 text-white shadow-lg shadow-violet-500/20' : 'bg-neutral-900 text-white shadow-md'
                        : isDark ? 'border border-zinc-600 text-zinc-300 hover:border-zinc-500 hover:bg-zinc-700' : 'border border-neutral-200 text-neutral-700 hover:border-neutral-900 hover:bg-neutral-100'
                    }`}
                  >
                    {time}
                  </button>
          ...
          <label className={`flex items-start gap-3 cursor-pointer ${isDark ? 'text-zinc-300' : ''}`}>
            ...
            <span className="text-sm">
              J&apos;accepte les{' '}
              <a href={LANDING_TERMS_URL} target="_blank" rel="noopener noreferrer" className="underline font-medium">conditions générales de vente</a> et la{' '}
          ...
          {submitError && (
            <p className={`text-sm px-4 py-2 rounded-lg ${isDark ? 'text-red-300 bg-red-500/10' : 'text-zinc-600 bg-zinc-100'}`}>{submitError}</p>
          )}
