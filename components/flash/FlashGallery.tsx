...
export const FlashGallery: React.FC<FlashGalleryProps> = ({
  ...
  const handleSaveFlash = () => {
    ...
    const data: Omit<FlashDesign, 'id' | 'createdAt'> = {
      ...
      imageUrl:
        form.imageUrl || 'https://images.unsplash.com/photo-1611501275019-9b5cda994e8d?w=400',
      price: form.price,
    ...
    <div className="min-h-full bg-zinc-50/50 dark:bg-zinc-950">
      <div className="space-y-6 animate-fade-in p-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              ...
              <div className="flex rounded-xl border border-zinc-200 dark:border-zinc-800 dark:border-zinc-800 overflow-hidden bg-white dark:bg-zinc-900">
                <button
                  ...
                  className={`p-3 transition-all ${viewMode === 'grid' ? 'bg-blue-600 text-white' : 'text-zinc-500 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800'}`}
                >
                  <Grid3X3 className="w-5 h-5" />
                </button>
                <button
                  onClick={() => setViewMode('list')}
                  className={`p-3 transition-all ${viewMode === 'list' ? 'bg-blue-600 text-white' : 'text-zinc-500 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800'}`}
                >
          ...
          {onAddFlash && (
            <button
              ...
              className="flex items-center gap-2 px-5 py-3 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition-all active:scale-[0.98] shadow-[0_2px_10px_-3px_rgba(37,99,235,0.3)]"
            >
        ...
        <div className="text-sm text-zinc-500 dark:text-zinc-400">
          {filteredDesigns.length} design{filteredDesigns.length > 1 ? 's' : ''} disponible
          {filteredDesigns.length > 1 ? 's' : ''}
        </div>

        {filteredDesigns.length === 0 ? (
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 dark:border-zinc-800 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)]">
            <EmptyState
              icon={Sparkles}
              title={designs.length === 0 ? 'Aucun flash pour le moment' : 'Aucun résultat'}
              description={
                designs.length === 0
              ...
              primaryAction={
                designs.length === 0 && onAddFlash
                  ? { label: 'Ajouter mon premier flash', onClick: openAddModal }
                  : designs.length > 0 || !onAddFlash
                    ? {
              ...
              viewMode === 'grid' ? (
                ...
                >
                  ...
                  <div className="relative aspect-square overflow-hidden bg-zinc-100 dark:bg-zinc-800">
                    ...
                    {design.reserved && (
                      <div
                        className={`absolute ${design.featured ? 'top-10' : 'top-2.5'} right-2.5 bg-black/70 backdrop-blur-sm text-white px-2.5 py-1 rounded-full text-[10px] font-semibold tracking-wide`}
                      >
                        Réservé
                      </div>
                    )}
                  ...
                  <div className="p-3">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleBookNow(design);
                      }}
                      disabled={design.reserved}
                ...
                <div
                  key={design.id}
                  className="flex items-center gap-5 p-5 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 dark:border-zinc-800 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] hover:shadow-[0_4px_20px_-5px_rgba(6,81,237,0.1)] transition-all cursor-pointer"
                >
                  <div className="relative w-28 h-28 rounded-xl overflow-hidden flex-shrink-0 bg-zinc-100 dark:bg-zinc-800">
                    ...
                    {design.reserved && (
                      <div className="absolute inset-0 bg-zinc-500/30 flex items-center justify-center">
                        <span className="bg-zinc-600 text-white px-2 py-0.5 rounded text-xs font-bold">
                          Réservé
                        </span>
                      </div>
                    )}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-sm text-zinc-500 dark:text-zinc-400 mt-1">
                      ...
                      <span className="px-2 py-0.5 bg-blue-50 dark:bg-blue-500/10 rounded-lg text-xs text-blue-600 dark:text-blue-400">
                        {design.category}
                      </span>
                    </div>
                  </div>
                      ...
                      <>
                        {onUpdateFlash && (
                          <button
                            ...
                            className="p-2 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-all active:scale-[0.98]"
                          >
                        ...
                        {onDeleteFlash && (
                          <button
                            onClick={() => handleDeleteFlash(design.id)}
                            className="p-2 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-zinc-500 dark:text-zinc-400 transition-all active:scale-[0.98]"
                          >
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                {selectedDesign.description && (
                  <p className="text-zinc-500 dark:text-zinc-400">{selectedDesign.description}</p>
                )}
                <div className="grid grid-cols-2 gap-4 py-4 border-y border-zinc-200 dark:border-zinc-800 dark:border-zinc-800">
                  <div>
                    <div className="text-sm text-zinc-500 dark:text-zinc-400 mb-1">Prix</div>
                  ...
                  <div>
                    ...
                    <div className="text-lg font-semibold text-zinc-900 dark:text-white tabular-nums">
                      {selectedDesign.estimatedDuration}min
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-zinc-500 dark:text-zinc-400 mb-1">Taille</div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedDesign.placement.map((place) => (
                      <span
                        key={place}
                        className="px-3 py-1 bg-blue-50 dark:bg-blue-500/10 rounded-lg text-sm text-blue-700 dark:text-blue-300"
                      >
                        {place}
                      </span>
                    ))}
                ...
                <button
                  onClick={() => handleBookNow(selectedDesign)}
                  disabled={selectedDesign.reserved}
                  ...
                  <div className="flex gap-2 pt-4 border-t border-zinc-200 dark:border-zinc-800 dark:border-zinc-800">
                    {onUpdateFlash && (
                      <button
                        ...
                        className="flex-1 py-2.5 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 dark:border-zinc-800 font-semibold text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
                      >
                    ...
                    {onDeleteFlash && (
                      <button
                        onClick={() => handleDeleteFlash(selectedDesign.id)}
                        className="flex-1 py-2.5 rounded-xl bg-red-50 dark:bg-red-950/30 text-red-600 font-semibold hover:bg-red-100 dark:hover:bg-red-950/50 flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
                      >
        ...
        {showFormModal && (onAddFlash || onUpdateFlash) && (
          <Modal
            ...
            title={editingFlash ? 'Modifier le flash' : 'Nouveau flash'}
            size="lg"
          >
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 min-h-[48px] border border-zinc-200 dark:border-zinc-800 dark:border-zinc-800 rounded-xl bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 border border-zinc-200 dark:border-zinc-800 dark:border-zinc-800 rounded-xl bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white placeholder:text-zinc-400 min-h-[80px] focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, depositAmount: Number(e.target.value) }))
                    }
                    className="w-full px-4 py-3 min-h-[48px] border border-zinc-200 dark:border-zinc-800 dark:border-zinc-800 rounded-xl bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, estimatedDuration: Number(e.target.value) }))
                    }
                    className="w-full px-4 py-3 min-h-[48px] border border-zinc-200 dark:border-zinc-800 dark:border-zinc-800 rounded-xl bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                  />
                </div>
              ...
              <div className="flex justify-end gap-3 pt-4">
                <button
                  ...
                  className="min-h-[44px] px-6 py-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 dark:border-zinc-800 rounded-xl font-semibold text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-all active:scale-[0.98]"
                >
                  Annuler
                </button>
                <button
                  onClick={handleSaveFlash}
                  disabled={savingFlash || !form.title.trim()}
                  className="min-h-[44px] px-6 py-3 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
                >
                  {savingFlash ? (
                    <>
