...
export const OnboardingEstablishmentStep: React.FC<OnboardingEstablishmentStepProps> = ({
  ...
  const saveFromPaste = useCallback(async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? formatGooglePlacesInvokeError(e.message) : 'Enregistrement impossible');
    } finally {
      setSaving(false);
    }
  ...
  const searchByName = async () => {
    ...
    try {
      ...
      if (list.length === 0) toast.error('Aucun résultat — essaie un autre nom ou colle l’URL Maps.');
    } catch (e) {
      toast.error(e instanceof Error ? formatGooglePlacesInvokeError(e.message) : 'Recherche indisponible');
    } finally {
      setSearching(false);
    }
  };
  ...
  const requestGeolocationHint = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setGeoLoading(false);
        toast.error('Accès à la position refusé — tu peux coller l’URL de ta fiche ou chercher par nom.');
      },
    ...
    >
      <div className="flex-1 flex flex-col min-h-0 max-h-full overflow-y-auto overscroll-y-contain touch-pan-y [-webkit-overflow-scrolling:touch]">
        ...
        <div className="flex-1 flex items-center justify-center px-4 sm:px-6 lg:px-10 py-5 sm:py-8 safe-bottom min-h-0">
          ...
          >
            ...
            <h1 id="est-title" className="text-xl sm:text-2xl font-bold tracking-tight text-zinc-900 dark:text-white mb-1.5">
              Localisation & fiche Google
            </h1>
            <p className="text-zinc-500 dark:text-zinc-400 text-sm mb-5 sm:mb-6">
              Relie ta fiche Google Maps pour afficher la carte et les avis sur ta vitrine publique. Tu pourras la modifier
              plus tard dans Paramètres → Établissement.
            </p>
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/80 dark:bg-zinc-900/40 p-4">
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-3">
                  On cherche avec le nom de ton studio : <span className="font-medium text-zinc-700 dark:text-zinc-300">{studioNameHint || '—'}</span>
                </p>
                    ...
                    {hits.map((h) => (
                      <li key={h.placeId}>
                        ...
                        >
                          ...
                          <span className="block text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">{h.formattedAddress}</span>
                        </button>
                      </li>
                    ))}
                  </ul>
                ...
                {hits.length > 0 && (
                  ...
                  >
                    {saving ? <Loader2 className="w-4 h-4 animate-spin inline" /> : null} Utiliser cette fiche
                  </button>
                )}
              </div>
              ...
              <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 p-4">
                ...
                >
                  {saving ? <Loader2 className="w-4 h-4 animate-spin inline" /> : null} Enregistrer ce lien
                </button>
              </div>
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 z-10 px-10 pb-10 pt-16 pointer-events-none">
          ...
          <p className="text-white text-base [text-shadow:0_2px_6px_rgba(0,0,0,0.8)]">Les clients te trouvent plus vite.</p>
        </div>
      </motion.div>
    </motion.div>
  );
