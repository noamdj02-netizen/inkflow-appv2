...
export const ReferralSuccessBanner: React.FC<ReferralSuccessBannerProps> = ({ studioId, useSupabase }) => {
  const [referrals, setReferrals] = useState<{ refereeStudioName: string | null; status: string }[]>([]);
  ...
  return (
    <div className="mb-4 sm:mb-6 rounded-2xl bg-gradient-to-r from-emerald-500/15 to-emerald-600/10 border border-emerald-200 dark:border-emerald-800 p-4 sm:p-5 flex items-start gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <a
          href="/referral"
          className="inline-block mt-2 text-sm font-semibold text-emerald-700 dark:text-emerald-300 hover:underline"
        >
      ...
      <button
        ...
        className="flex-shrink-0 p-2 rounded-lg text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/20 transition-colors"
        aria-label="Fermer"
      >
        <X className="w-5 h-5" />
      </button>
