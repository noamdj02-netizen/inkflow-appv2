    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[var(--text-secondary)] text-sm mt-1">{waiting.length} en attente, {notified.length} notifie(s), {booked.length} reserve(s)</p>
        </div>
        <button onClick={() => setShowAdd(true)} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-500 dark:bg-blue-500 dark:hover:bg-blue-600">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div>
          ...
          <div className="space-y-3">
            {waiting.map(entry => (
              <div key={entry.id} className="bg-[var(--bg-card)] rounded-xl p-4 border border-[var(--border)]">
                <div className="font-semibold text-sm text-[var(--text-primary)]">{entry.clientName}</div>
                <div className="text-xs text-[var(--text-secondary)]">{entry.clientEmail}</div>
                {entry.desiredService && <div className="text-xs text-[var(--text-tertiary)] mt-1">{entry.desiredService}</div>}
                {entry.preferredDates && <div className="text-xs text-[var(--text-tertiary)]">Dates souhaitees: {entry.preferredDates}</div>}
                <div className="flex gap-2 mt-3">
                  ...
                  >
                    {action?.id === entry.id && action?.type === 'remove' ? <Loader2 className="w-3 h-3 animate-spin text-[var(--text-secondary)]" /> : <X className="w-3 h-3 text-[var(--text-secondary)]" />}
                  </button>
                </div>
              </div>
            ))}
            {waiting.length === 0 && <p className="text-sm text-[var(--text-tertiary)] text-center py-4">Aucun</p>}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {notified.map(entry => (
              <div key={entry.id} className="bg-[var(--bg-card)] rounded-xl p-4 border border-[var(--border)]">
                <div className="font-semibold text-sm text-[var(--text-primary)]">{entry.clientName}</div>
                <div className="text-xs text-[var(--text-secondary)]">{entry.clientEmail}</div>
                {entry.notifiedAt && <div className="text-xs text-[var(--text-tertiary)] mt-1">Notifie le {new Date(entry.notifiedAt).toLocaleDateString('fr-FR')}</div>}
                <div className="flex gap-2 mt-3">
                  <button
            ...
            {notified.length === 0 && <p className="text-sm text-[var(--text-tertiary)] text-center py-4">Aucun</p>}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {booked.map(entry => (
              <div key={entry.id} className="bg-[var(--bg-card)] rounded-xl p-4 border border-blue-500/30">
                <div className="font-semibold text-sm text-[var(--text-primary)]">{entry.clientName}</div>
                <div className="text-xs text-[var(--text-secondary)]">{entry.clientEmail}</div>
                {entry.desiredService && <div className="text-xs text-blue-600 dark:text-blue-400 mt-1">{entry.desiredService}</div>}
              </div>
            ))}
            {booked.length === 0 && <p className="text-sm text-[var(--text-tertiary)] text-center py-4">Aucun</p>}
          </div>
        </div>
      </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-[var(--bg-card)] rounded-2xl max-w-md w-full p-6 border border-[var(--border)]" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-[var(--text-primary)]">Ajouter a la liste d'attente</h3>
              <button onClick={() => setShowAdd(false)} className="p-2 rounded-lg hover:bg-[var(--bg-hover)]"><X className="w-5 h-5 text-[var(--text-secondary)]" /></button>
            </div>
            <div className="space-y-4">
              <input type="text" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                placeholder="Nom du client" className="w-full px-4 py-3 border border-[var(--border)] rounded-xl bg-[var(--bg-primary)] text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-blue-500/50" />
              ...
              <input type="text" value={form.service} onChange={e => setForm(p => ({ ...p, service: e.target.value }))}
                placeholder="Service souhaite (optionnel)" className="w-full px-4 py-3 border border-[var(--border)] rounded-xl bg-[var(--bg-primary)] text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-blue-500/50" />
              <input type="text" value={form.dates} onChange={e => setForm(p => ({ ...p, dates: e.target.value }))}
                placeholder="Dates preferees (optionnel)" className="w-full px-4 py-3 border border-[var(--border)] rounded-xl bg-[var(--bg-primary)] text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-blue-500/50" />
              <textarea value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))}
                placeholder="Notes (optionnel)" rows={3} className="w-full px-4 py-3 border border-[var(--border)] rounded-xl resize-none bg-[var(--bg-primary)] text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-blue-500/50" />
