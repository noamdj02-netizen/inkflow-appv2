...
const MODULE_ROWS: {
  ...
  { id: 'planning', label: 'Planning & rendez-vous', description: 'Agenda, vues semaine / mois et créneaux.', Icon: Calendar },
  { id: 'finance', label: 'Finance', description: 'Revenus, acomptes et caisse.', Icon: Wallet },
  { id: 'flash_shop', label: 'Galerie flash & portfolio', description: 'Flashs, vitrine liée et médias.', Icon: Sparkles },
  { id: 'vitrine', label: 'Page vitrine & app client', description: 'Page publique, app client et personnalisation.', Icon: Globe },
  { id: 'loyalty', label: 'Fidélité', description: 'Points et récompenses clients.', Icon: Star },
  { id: 'consent_forms', label: 'Consentements', description: 'Modèles de formulaires de consentement.', Icon: FileCheck },
  { id: 'healing_followup', label: 'Soins post-tattoo', description: 'Fiches de soins et suivi après séance.', Icon: Heart },
];
...
export const ModulesSettings: React.FC<ModulesSettingsProps> = ({ studioId, value, onChange }) => {
  ...
  return (
    <div className="space-y-6 max-w-2xl w-full">
      <div>
        <h2 className="font-display text-2xl sm:text-3xl font-bold tracking-tight text-zinc-900 dark:text-white">Modules</h2>
        <p className="text-zinc-500 dark:text-zinc-400 text-sm sm:text-base mt-1.5 max-w-2xl">
          Activez ou masquez les blocs du menu et les sections associées. Les données restent en base ; seule la navigation est adaptée.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 divide-y divide-zinc-100 dark:divide-zinc-800 overflow-hidden">
        {MODULE_ROWS.map(({ id, label, description, Icon }) => {
          ...
          return (
            <div key={id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-4 sm:p-5">
      ...
      <div className="flex justify-end">
        <button
          ...
          onClick={handleSave}
          disabled={saving}
          className="inline-flex items-center justify-center px-5 py-2.5 rounded-xl bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 font-semibold text-sm hover:opacity-90 transition-all active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none"
        >
