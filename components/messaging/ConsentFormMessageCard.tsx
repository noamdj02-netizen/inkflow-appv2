...
export const ConsentFormMessageCard: React.FC<ConsentFormMessageCardProps> = ({
  ...
  const load = useCallback(async () => {
    ...
    const { data, error: err } = await supabase
      ...
      .select('template, client_name, signed_at')
      .eq('id', consentFormId)
      .maybeSingle();
    if (err) {
  ...
  if (loading) {
    return (
      <div className={`rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-6 flex items-center justify-center gap-2 text-sm text-zinc-500 ${className}`}>
  ...
  if (error || !template) {
    return (
      <div className={`rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 ${className}`}>
        {error ?? 'Erreur'}
      </div>
    );
  }
  ...
  if (mode === 'studio_status') {
    return (
      <div className={`rounded-2xl border border-blue-200/90 dark:border-blue-500/25 bg-blue-50/90 dark:bg-blue-500/10 px-4 py-3 space-y-2 min-w-0 max-w-full select-text ${className}`}>
        <div className="flex items-start gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-zinc-900 dark:text-white leading-snug">{title}</p>
            {signedAt ? (
              ...
              <p className="text-xs text-amber-900 dark:text-amber-100/90 mt-1">En attente de signature client</p>
            )}
          </div>
        </div>
        <a
  ...
  if (signedAt) {
    return (
      <div className={`rounded-2xl border border-emerald-200 bg-emerald-50/90 px-4 py-3 text-sm text-emerald-900 min-w-0 max-w-full ${className}`}>
  ...
  return (
    <div className={`rounded-2xl border border-neutral-200 bg-white shadow-sm overflow-hidden min-w-0 max-w-full ${className}`}>
          ...
          onSign={async ({ signatureData, filledTemplateText }) => {
            const { error: upErr } = await supabase
              ...
              .update({
                ...
                filled_template_text: filledTemplateText,
              })
              .eq('id', consentFormId);
