...
export const MessagingTab: React.FC<MessagingTabProps> = ({
  ...
  const renderStructuredOrText = (msg: Message, isOutbound: boolean) => {
    ...
    if (structured?.kind === 'payment_card') {
      return (
        <div className="rounded-2xl border border-slate-200/80 dark:border-zinc-700 border-l-4 border-l-amber-500 bg-white dark:bg-zinc-800/90 p-4 space-y-3 shadow-sm">
    ...
    if (structured?.kind === 'payment_receipt') {
      return (
        <div className="rounded-2xl border border-slate-200/80 dark:border-zinc-700 border-l-4 border-l-emerald-500 bg-white dark:bg-zinc-800/90 p-4 space-y-2 shadow-sm">
          <div className="flex items-center gap-2 text-slate-900 dark:text-white">
          ...
          {structured.receiptUrl ? (
            ...
            <p className="text-xs text-slate-500 dark:text-zinc-500">Ton acompte a bien été enregistré.</p>
          )}
        </div>
      );
    }
  ...
  return (
      ...
      <div className="flex h-full">
        ...
        {selectedThreadId && selectedThread ? (
          <main className="flex-1 flex flex-col min-w-0 min-h-0">
            {/* Header Chat */}
            <header className="flex flex-col gap-2 px-4 sm:px-5 py-3 sm:py-4 border-b border-slate-200/80 dark:border-zinc-800 bg-white dark:bg-zinc-900">
              <div className="flex items-start gap-3 min-w-0">
                <button
                  type="button"
                  onClick={() => setSelectedThreadId(null)}
                  aria-label="Retour à la liste des conversations"
                  className="md:hidden shrink-0 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-xl hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors active:scale-95"
                >
                  <ArrowLeft className="w-5 h-5 text-slate-500 dark:text-zinc-400" />
                </button>

                <div className="w-10 h-10 shrink-0 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    {healthLine === 'none' && (
                      <span className="inline-flex items-center rounded-lg border border-slate-200/90 dark:border-zinc-700 bg-slate-50/80 dark:bg-zinc-800/80 px-2 py-1 text-[11px] text-slate-500 dark:text-zinc-400">
                    ...
                    {selectedThreadId.startsWith('pr_') && onOpenLinkedProjectRequest && (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 dark:border-zinc-600 bg-white dark:bg-zinc-800 px-2.5 py-1.5 text-[11px] font-medium text-slate-800 dark:text-zinc-200 hover:bg-slate-50 dark:hover:bg-zinc-700 active:scale-[0.98] transition-all min-h-[36px]"
                      >
                ...
                <button
                  ...
                  className="shrink-0 min-w-[40px] min-h-[40px] flex items-center justify-center rounded-xl hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors"
                >
              ...
              <div className="flex items-center gap-3">
                {selectedThreadId?.startsWith('pr_') ? (
                  <button
                    ...
                    className="p-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-zinc-800 text-slate-500 dark:text-zinc-400 hidden sm:flex active:scale-[0.98] transition-all"
                  >
          ...
          <main className="flex-1 flex flex-col min-w-0 min-h-0 items-center justify-center bg-slate-50/50 dark:bg-zinc-950/50 p-6">
            {fallbackThreadLoading ? (
              ...
              <div className="text-center max-w-xs">
                ...
                <button
                  type="button"
                  onClick={() => setSelectedThreadId(null)}
                  className="inline-flex items-center justify-center min-h-[44px] px-4 rounded-xl border border-slate-200 dark:border-zinc-700 text-sm font-medium text-slate-800 dark:text-zinc-200 hover:bg-slate-100 dark:hover:bg-zinc-800 active:scale-[0.98] transition-all"
                >
                  Retour à la liste
                </button>
              </div>
    ...
    >
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-xs font-medium text-slate-500 dark:text-zinc-500 mb-1.5">
            Montant (€)
          </label>
          <input
            type="text"
            ...
            className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20"
            placeholder="50"
          />
        </div>
        <div className="flex gap-2 justify-end pt-2">
          <button
            ...
            className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-zinc-700 text-sm font-medium text-slate-700 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-zinc-800 disabled:opacity-50 active:scale-[0.98] transition-all"
          >
            Annuler
          </button>
          <button
