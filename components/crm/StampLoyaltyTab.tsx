...
export const StampLoyaltyTab: React.FC<StampLoyaltyTabProps> = ({ studioId, clients }) => {
  ...
  const [stateByClient, setStateByClient] = useState<
    Record<string, { stampsInCycle: number; totalCompletedTattoos: number }>
  >({});
  ...
  if (!studioId) {
    return (
      <div className="type-body text-muted-foreground p-6">
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      <div>
        <h1 className="type-heading">Fidélité — Carte à tampons</h1>
        <p className="type-subtitle mt-1.5 max-w-2xl">
          Définissez combien de séances terminées déclenchent un montant offert. Un code promo
          unique est envoyé au client par email ; vous êtes alerté sur les prochaines demandes de
          RDV.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 p-6 space-y-5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="type-heading-sm">Paramètres</h2>
        </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setSettings((p) => ({
                  ...p,
                  tattoosRequired: Math.max(1, parseInt(e.target.value, 10) || 1),
                }))
              }
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setSettings((p) => ({
                  ...p,
                  rewardEuros: Math.max(1, parseInt(e.target.value, 10) || 1),
                }))
              }
              className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white"
            />
        ...
        <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
          Chaque fois que vous passez un rendez-vous en « Terminé », un tampon est ajouté. Au{' '}
          {settings.tattoosRequired}
          e, un code promo <strong>INK-…</strong> de {settings.rewardEuros}€ est créé et envoyé au
          client par email.
        </p>
