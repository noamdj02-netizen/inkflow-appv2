...
export const PricingSection: React.FC = () => {
  ...
  const plans = [
    {
      id: 'solo' as const,
      name: 'Solo',
      descriptionKey: 'pricing.soloDesc',
      priceMonthly: 29,
      priceAnnual: 24,
      featureKeys: [
        'pricing.f1',
  ...
  return (
    <section
      id="pricing"
      className={`py-16 sm:py-24 px-4 sm:px-6 lg:px-8 bg-white text-neutral-900 transition-opacity duration-700 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
    >
      <div
        ref={ref}
          ...
          <div className="mb-12 sm:mb-16 p-6 sm:p-8 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-left max-w-2xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8">
              <div className="p-4 rounded-xl bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700">
                ...
                <p className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">
                  {gainTemps}h <span className="text-sm font-normal text-zinc-500">/semaine</span>
                </p>
              ...
              <div className="p-4 rounded-xl bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700">
                ...
                <p className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">
                  {noShowEvites} <span className="text-sm font-normal text-zinc-500">/mois</span>
                </p>
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                  grâce aux acomptes auto
                </p>
              </div>
              <div className="p-4 rounded-xl bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700">
                <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase mb-1">
            ...
            <div className="mt-8 pt-6 border-t border-zinc-200 dark:border-zinc-700 text-center">
              <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-4">
                Pour seulement{' '}
                <strong className="text-zinc-900 dark:text-zinc-100">{PRO_PLAN_PRICE}€/mois</strong>
                , Inkflow vous rapporte en moyenne{' '}
                <strong className="text-blue-600 dark:text-blue-400">{roi}x</strong> son coût.
              </p>
              <a
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-10 sm:mb-12">
          {plans.map((plan, index) => (
            ...
            >
              ...
              <div className="mb-8">
                <h3
                  className={`text-2xl font-bold mb-2 ${plan.popular ? 'text-white' : 'text-neutral-900'}`}
                >
                  {plan.name}
                </h3>
                ...
                {plan.priceMonthly ? (
                  <>
                    <div className="flex items-baseline gap-2">
                      <span
                        className={`text-5xl font-bold ${plan.popular ? 'text-white' : 'text-neutral-900'}`}
                      >
                    ...
                    {isAnnual && plan.priceAnnual && (
                      <div
                        className={`text-sm mt-2 ${plan.popular ? 'text-neutral-400' : 'text-neutral-500'}`}
                      >
              ...
              <div className="space-y-4">
                {plan.featureKeys.map((key, featureIndex) => (
                  <div key={featureIndex} className="flex items-start gap-3">
                    ...
                    />
                    <span
                      className={`text-sm ${plan.popular ? 'text-neutral-200' : 'text-neutral-700'}`}
                    >
                      {t(key)}
                    </span>
                  </div>
