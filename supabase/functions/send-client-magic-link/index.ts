...
const CLIENT_ONBOARD_AFTER_AUTH = "/discover";
...
function sanitizeRedirectTo(input: string, appBase: string): string {
  ...
  const fallback = `${base}/auth/callback?redirect_to=${encodeURIComponent(CLIENT_ONBOARD_AFTER_AUTH)}`;
  try {
  ...
  try {
    ...
    const rawRedirect =
      typeof body.redirectTo === "string"
        ? body.redirectTo
        : `${APP_URL}/auth/callback?redirect_to=${encodeURIComponent(CLIENT_ONBOARD_AFTER_AUTH)}`;
