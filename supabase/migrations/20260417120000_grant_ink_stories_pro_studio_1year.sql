-- InkFlow : plan Studio gratuit 1 an pour ink.stories.pro@gmail.com (Paris)
-- Prolonge / remplace la promo précédente (5 mois) : fenêtre d’au moins 12 mois à partir de l’application.
...
-- 1. Studio : actif, plan commercial studio, essai / fenêtre alignée sur +1 an
UPDATE inkflow_studios
SET
  subscription_status = 'active',
  plan_type = 'studio',
  trial_ends_at = GREATEST(COALESCE(trial_ends_at, NOW()), NOW() + INTERVAL '1 year'),
  updated_at = NOW()
...
-- 3. Subscription existante (autre id) : aligner plan Studio + prolonger d’au moins 1 an
UPDATE inkflow_subscriptions sub
SET
  plan = 'studio',
  status = 'active',
  current_period_end = GREATEST(COALESCE(sub.current_period_end, NOW()), NOW() + INTERVAL '1 year'),
  updated_at = NOW()
FROM inkflow_studios s
WHERE sub.studio_id = s.id
