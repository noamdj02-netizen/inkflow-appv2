-- Résolution publique par slug insensible à la casse (ex. Jade vs jade).
-- Garantit la colonne discover/vitrine si une migration antérieure n’a pas été appliquée.
ALTER TABLE public.inkflow_studios
...
COMMENT ON FUNCTION public.get_studio_public_by_slug(text) IS
  'Lecture publique studio par slug (comparaison insensible à la casse).';
