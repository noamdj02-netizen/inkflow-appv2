...
function ClientOnboarding({ onDone }: { onDone: () => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Content bottom */}
      <div
        className="absolute inset-x-0 bottom-0 px-6 z-10"
              ...
              {ONBOARD_SLIDES.map((_, i) => (
                <button
                  ...
                  style={{
                    width: i === slide ? 20 : 6,
                    height: 6,
                    background: i === slide ? clientAccent : 'rgba(255,255,255,0.35)',
                  }}
                />
              ))}
            </div>
...
export const LoginPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return;
      const target =
        user && (await shouldRedirectPortalClientFromProDashboard(user))
          ? pathForClientDashboardTab('home')
          : path;
    ...
    <>
      <AnimatePresence>
        {showOnboarding && (
          <ClientOnboarding
            onDone={() => {
              window.location.href = pathForClientDashboardTab('home');
            }}
          />
        )}
      </AnimatePresence>
