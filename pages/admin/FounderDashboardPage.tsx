...
import { getFounderNavMeta, isFounderAdminSlug, type FounderAdminSlug } from '../../lib/founderAdminNav';
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 pr-10">
        <p className={cn('flex min-w-0 items-center gap-2 text-[11px] font-medium uppercase tracking-wide', labelClass)}>
          {props.icon ? <span className="shrink-0 [&_svg]:text-current">{props.icon}</span> : null}
          <span className="leading-tight">{props.label}</span>
        </p>
        {props.sub ? <p className="mt-1.5 text-[11px] leading-snug text-zinc-600">{props.sub}</p> : null}
        <div
          className={cn(
...
function maskSensitiveNumber(reveal: boolean, value: number, suffix = ''): React.ReactNode {
  if (!reveal) {
    return <span className="inline-block blur-[6px] select-none tabular-nums text-zinc-900">••••{suffix}</span>;
  }
  return (
...
function maskEuro(reveal: boolean, value: number): React.ReactNode {
  if (!reveal) {
    return <span className="inline-block blur-[6px] select-none tabular-nums text-zinc-900">•••• €</span>;
  }
  return (
    <span className="tabular-nums text-zinc-900">
    ...
    <>
      <SEO
        ...
        description={
          ...
          'Tableau de bord interne InkFlow — MRR SaaS, studios, activité'
        }
        noindex
        canonical={seoPath}
      />
          ...
          <div className="space-y-3 rounded-2xl border border-amber-200 bg-amber-50/90 px-4 py-5 text-sm text-amber-950 sm:px-6">
            {serverAccessIssue === 'server_env_missing' ? (
              <>
                ...
                <p className="leading-relaxed text-amber-900/90">
                  Définis <code className="rounded bg-amber-100/80 px-1.5 py-0.5 text-xs">FOUNDER_ADMIN_EMAILS</code> sur le projet Supabase
            ...
            {serverAccessIssue === 'forbidden' ? (
              <>
                ...
                <p className="leading-relaxed text-amber-900/90">
                  L’e-mail de ta session n’est pas dans <code className="rounded bg-amber-100/80 px-1.5 py-0.5 text-xs">FOUNDER_ADMIN_EMAILS</code>
        ...
        {data && !serverAccessIssue ? (
          <>
            ...
            <div className="founder-print-only text-black space-y-3 pb-4 mb-4 border-b border-zinc-300">
              ...
              {sectionSlug && navMeta ? (
                <p className="text-sm font-medium text-zinc-800">Vue exportée : {navMeta.pageTitle}</p>
              ) : null}
            ...
            <section aria-labelledby="founder-saas" className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 xl:grid-cols-4">
                <FounderKpiCard
                  ...
                  icon={<Wallet className="h-4 w-4 text-amber-600" aria-hidden />}
                ...
                <FounderKpiCard
                  ...
                  onExportCsv={() =>
                    downloadCsv('founder-kpi-arr.csv', ['metric', 'value'], [
                ...
                >
                  {revealSensitive ? (
                    ...
                    <span className="inline-block select-none blur-[6px] tabular-nums">•••• €</span>
                  )}
                </FounderKpiCard>
                <FounderKpiCard
                  ...
                  onExportCsv={() =>
                    downloadCsv('founder-kpi-abo-actifs.csv', ['metric', 'value'], [
            ...
            <section aria-labelledby="founder-health" className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <FounderKpiCard
                  ...
                  onExportCsv={() =>
                    downloadCsv('founder-health-failed.csv', ['metric', 'value'], [
                ...
                <FounderKpiCard
                  ...
                  onExportCsv={() =>
                    downloadCsv('founder-health-pending-stale.csv', ['metric', 'value'], [
            ...
            <section aria-labelledby="founder-volume" className="space-y-3">
              <h2 id="founder-volume" className="text-sm font-semibold uppercase tracking-wide text-zinc-700">
            ...
            <details className="group rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-5">
              <summary className="flex min-h-[44px] cursor-pointer list-none items-center justify-between gap-2 text-sm font-medium text-zinc-800">
                ...
                <span className="hidden text-xs text-zinc-500 group-open:inline">Masquer</span>
              </summary>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-zinc-200 bg-zinc-50/80 p-4">
                <div>
                  <p className="text-xs font-medium uppercase tracking-wide text-zinc-600">Total acomptes (mois en cours)</p>
                  <p
                    className={cn(
                      'mt-1 text-2xl font-bold tabular-nums',
                      revealSensitive && data.kpis.depositsMonthEur === 0 && 'font-semibold text-zinc-400 [&_span]:text-zinc-400',
                    )}
                  >
            ...
            >
              <h2 id="founder-ops" className="flex items-center gap-2 text-sm font-semibold text-zinc-800">
              ...
              <ul className="space-y-2 text-sm leading-relaxed text-zinc-700">
                <li>
                  ...
                  >
                    Supabase Dashboard <ExternalLink className="h-3.5 w-3.5 shrink-0 opacity-70" aria-hidden />
                  </a>{' '}
                ...
                <li>
                  <a
                    ...
                    className="inline-flex items-center gap-1 font-medium text-amber-800 underline-offset-2 hover:underline"
                  >
                    Sentry <ExternalLink className="h-3.5 w-3.5 shrink-0 opacity-70" aria-hidden />
                  </a>{' '}
                  — erreurs front (`ErrorBoundary`) et Edge (`stripe-webhook`) si DSN configuré.
                </li>
                <li>Dashboard Stripe — paiements Connect & abonnements SaaS (projet InkFlow).</li>
            ...
            <div id="founder-charts" className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 founder-print-hide scroll-mt-24">
              <WidgetShell
                ...
                <div className="mt-4 h-[160px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data.activity.onboardingStepDistribution} layout="vertical">
                      ...
                      <YAxis type="category" dataKey="step" width={100} tick={{ fill: '#71717a', fontSize: 11 }} />
                      <RechartsTooltip {...RECHARTS_TOOLTIP_LIGHT} />
            ...
            >
              <ul className="grid gap-3 text-sm sm:grid-cols-2">
                  ...
                  <span className="text-zinc-700">
                    ...
                    <span
                      className={cn(
                        ...
                        data.alerts.studiosStuckOnboarding === 0 ? 'text-zinc-400' : 'text-zinc-900',
                      )}
                    >
                ...
                <li className="flex gap-3 rounded-xl border border-red-200 bg-red-50/90 px-4 py-3">
                  ...
                  <span className="text-zinc-700">
                    ...
                    <span
                      className={cn(
                        'tabular-nums text-lg font-semibold',
                        data.alerts.unpaidDepositsOver48h === 0 ? 'text-zinc-400' : 'text-zinc-900',
                      )}
                    >
                ...
                <li className="flex gap-3 rounded-xl border border-amber-200 bg-white px-4 py-3">
                  <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-600" aria-hidden />
                  <span className="text-zinc-700">
                    <strong className="font-semibold text-zinc-900">Sans flash &gt; 48h</strong> (cohorte 365j)
                    <br />
                    <span
                      className={cn(
                        'tabular-nums text-lg font-semibold',
                        data.alerts.studiosNoFlashAfter48h === 0 ? 'text-zinc-400' : 'text-zinc-900',
                      )}
                    >
                ...
                <li className="flex gap-3 rounded-xl border border-amber-200 bg-white px-4 py-3">
                  <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-600" aria-hidden />
                  <span className="text-zinc-700">
                    <strong className="font-semibold text-zinc-900">Stripe pas prêt &gt; 72h</strong> (cohorte 365j)
                    <br />
                    <span
                      className={cn(
                        'tabular-nums text-lg font-semibold',
                        data.alerts.studiosNoStripeAfter72h === 0 ? 'text-zinc-400' : 'text-zinc-900',
                      )}
                    >
                ...
                <li className="rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 sm:col-span-2">
                  <p className="text-xs leading-relaxed text-zinc-600">{data.alerts.suspiciousAuthNote}</p>
                </li>
                <li className="flex gap-3 rounded-xl border border-zinc-200 bg-white px-4 py-3 sm:col-span-2">
                  ...
                  <span className="text-zinc-700">
                    <strong className="font-semibold text-zinc-900">Studios inactifs 14j</strong> (updated_at){' '}
                    <span
                      className={cn(
            ...
            <div id="founder-growth" className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 founder-print-hide scroll-mt-24">
              <WidgetShell
              ...
              >
                <p
                  className={cn(
                    ...
                    data.growth.churnSubscriptionsMonth === 0 ? 'font-semibold text-zinc-400' : 'text-zinc-900',
                  )}
                >
