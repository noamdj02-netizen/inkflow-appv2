...
export const DebugExperiencePage: React.FC = () => {
  ...
  const run = async (
    action: 'welcome_pack' | 'day_notifications' | 'loyalty_only',
    loadingKey: string
  ) => {
  ...
  return (
    <div className="founder-admin-scroll-root bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 p-4 sm:p-8">
      <SEO
        title="Simulation Inkflow"
        description="Outil interne de test"
        noindex
        canonical="/admin/debug-experience"
      />
      <div className="max-w-xl mx-auto space-y-6">
        ...
        <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 p-6 sm:p-8 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <h1 className="type-heading">Mode Simulation Inkflow</h1>
          </div>
          <p className="type-subtitle mt-1.5">
            Recevez les mêmes emails que vos clients (Thomas / manchette / 450 €) pour valider le
            design sur mobile.
          </p>
        </div>
        ...
        {!allowed ? (
          <div className="rounded-2xl border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-950/30 p-6 text-sm text-amber-900 dark:text-amber-200">
            Accès réservé au compte de test produit ({SIMULATION_ALLOWED_EMAIL}). Vous êtes connecté
            en tant que <span className="font-mono">{user.email}</span>.
          </div>
        ) : (
          <div className="space-y-4">
            <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 p-6 space-y-4">
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-400">
                Confirmation RDV, rappel 24h, nouveau projet vitrine, nouvelle demande RDV, fidélité
                80 €, annulation.
              </p>
              <button
                type="button"
            ...
            <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 p-6 space-y-4">
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-400">
                5 notifications in-app (messages, paiement, rappels, demandes…) — visibles dans le
                dashboard.
              </p>
              <button
                type="button"
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-500 px-1">
              Sujets des mails de simulation contiennent souvent « [simulation] » pour les
              distinguer. Déployez la fonction Edge{' '}
              <code className="font-mono text-[11px]">simulate-inkflow-experience</code> si besoin.
            </p>
          </div>
        )}
      </div>
