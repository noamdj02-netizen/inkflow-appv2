...
| ----------------------------- | --- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| inkflow_studios               | ✅  | studios_select_own, studios_insert_own, studios_update_own (email = JWT) | **plus de SELECT anon sur toute la table** — lecture publique slug via RPC `get_studio_public_by_slug` uniquement |
...
## 3. Historique — fuite potentielle `studios_public_read_by_slug` (corrigé)

- **Constat initial** : l’ancienne policy **`studios_public_read_by_slug`** avec **`USING (true)`** permettait à un rôle **anon** de lire toute la table `inkflow_studios` (fuite multi-tenant dont **emails**).
- **Correctif en base** (voir migrations `20250309100000_security_harden_rls_public_studio.sql`, `20250313000000_rls_production_hardening.sql` et évolutions de `get_studio_public_by_slug`) : policy supprimée ; vitrine publique = **RPC `get_studio_public_by_slug(slug)`** (SECURITY DEFINER, colonnes contrôlées), pas de `SELECT` direct anon sur `inkflow_studios`.
- **À vérifier après chaque migration** : aucune nouvelle policy anon `USING (true)` sur `inkflow_studios` sans revue explicite.
